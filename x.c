...
void xresize(int col, int row)
{
    ...
    xw.buf = XCreatePixmap(
        xw.dpy, xw.win, win.w, win.h, DefaultDepth(xw.dpy, xw.scr));
...
void ximinstantiate(Display* dpy, XPointer client, XPointer call)
{
    if (ximopen(dpy))
        XUnregisterIMInstantiateCallback(
            xw.dpy, NULL, NULL, NULL, ximinstantiate, NULL);
}
...
int xmakeglyphfontspecs(
    XftGlyphFontSpec* specs, const Glyph* glyphs, int len, int x, int y)
{
    ...
    const XftGlyphFontSpec* specs, Glyph base, int len, int x, int y)
{
    ...
    if (base.mode & ATTR_UNDERLINE) {
        XftDrawRect(
            xw.draw, fg, winx, winy + win.cyo + dc.font.ascent + 1, width, 1);
    }
